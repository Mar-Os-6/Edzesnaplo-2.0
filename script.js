...
function renderQuickExercises() {
    ...
    const current = exKey($('#exercise').value);
    list.forEach(name => {
        const active = !quickEdit && current && exKey(name) === current;
        const chip = h('button', { type: 'button', class: 'chip small' + (active ? ' on' : ''), onClick: () => { if (!quickEdit) { $('#exercise').value = name; onExerciseChosen(); } } }, name);
        if (quickEdit) {
...
function onExerciseChosen() {
    ...
    updateLastBox(); refreshPlaceholders(); saveDraft(); renderQuickExercises();
}
...
function initLogger() {
    ...
    $('#exercise').addEventListener('input', () => {
        ...
        updateLastBox(); refreshPlaceholders(); renderQuickExercises();
    });
...
function renderStats() {
    ...
    more.onclick = () => { S.prAll = !S.prAll; renderStats(); };
...
function refreshAll() {
    renderTop(); renderWeek(); renderTemplates(); renderDayLog();
    updateLastBox(); refreshPlaceholders();
...
function init() {
    ...
    $('#chart-exercise-select').addEventListener('change', e => { S.ex = e.target.value; renderStats(); });
